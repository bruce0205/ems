...
module.exports = (app, db) => {
  ...
  async function fetchQualityYieldData (req) {
    const data = await db.query(`
      select dataHour, Yield_Target, Yield_Real,Hole1_Real,Hole2_Real,dataDate from GetOEE_Q_Yield('${req.query.mah_num}') order by dataHour
    `, {
        raw: false, // Set this to true if you don't have a model definition for your query.
        type: Sequelize.QueryTypes.SELECT
      })
    ...
    /**/
    return { label, targetData, realData,hole1Data,hole2Data }
  }
  ...
  router.get('/api/quality/yieldData', function (req, res, next) {
    ...
    `, {
        ...
        res.send({ label, targetData, realData,hole1Data,hole2Data });
      }).catch(err => {
        console.error(err);
      });
  })
  ...
  async function fetchPerformanceCtData (req) {
    const data = await db.query(`
      select dataHour, Output_Target, Output_Real, Output_Target_Acc, Output_Real_Acc, dataDate from GetOEE_P_CT('${req.query.mah_num}')  order by dataHour
    `, {
        raw: false, // Set this to true if you don't have a model definition for your query.
        type: Sequelize.QueryTypes.SELECT
      })
